/*------------------------------------------------------------------------------
	...
	Programador tarea 2A: annabel.pizarro@estudiants.urv.cat
...
void genera_sprites(char mat[][COLUMNS])
{
	...
	SPR_ocultarSprites(ROWS*COLUMNS); //Ocultamos todos los sprites
	
	for(int i=0; i<ROWS*COLUMNS; i++){
		vect_elem[i].ii = -1;			//Desactivamos elementos del vector
	}
	SPR_actualizarSprites(OAM, ROWS*COLUMNS);	//Actualizamos sprites - "Ocultamos sprites viejos"
	
	char mask = 0b00111;
	//Recorremos la matriz de juego, creando los elementos en caso que los haya
	for (int i=0; i<ROWS; i++){
		for(int j=0; j<COLUMNS;j++){
			char element = mask & mat[i][j];	//Aplicamos mascara al valor de la matriz
			if (element!=0 && element!=7){	//Si no es casilla vacia, bloque solido o hueco, creamos elemento
				if (crea_elemento(element, i,j)!=ROWS*COLUMNS) n_sprites++;
			}
		}
	}
	SPR_actualizarSprites(OAM, n_sprites);		//Actualizamos sprites - "Mostramos sprites nuevos"
}
...
// Tarea 2Aa:
	...
	dmaCopy(SpritesPal, (void *) 0x05000200, sizeof(SpritesPal)); //0x05000200 = @de la paleta de sprites
	dmaCopy(SpritesTiles, (void *) 0x06400000, sizeof(SpritesTiles)); //0x06400000 = @baldosas sprites
